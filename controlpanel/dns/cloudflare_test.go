...
func Test_CloudflareDNS_UpdateV6(t *testing.T) {
	httpmock.Activate()
	defer httpmock.DeactivateAndReset()

	httpmock.RegisterResponder(http.MethodGet, "http://cf/client/v4/zones/yyy/dns_records?match=all&name=game.example.com&page=1&per_page=100&proxied=false&type=AAAA",
		httpmock.NewJsonResponderOrPanic(http.StatusOK, map[string]any{
			"errors":   make([]any, 0),
			"messages": make([]any, 0),
			"result": []map[string]any{
	...
	err = dnsProvider.UpdateV6(context.Background(), net.ParseIP("2001:db8::2"))
	assert.NoError(t, err)

	assert.Equal(t, 1, httpmock.GetCallCountInfo()["GET http://cf/client/v4/zones/yyy/dns_records?match=all&name=game.example.com&page=1&per_page=100&proxied=false&type=AAAA"])
	assert.Equal(t, 1, httpmock.GetCallCountInfo()["PATCH http://cf/client/v4/zones/yyy/dns_records/record1"])
}
