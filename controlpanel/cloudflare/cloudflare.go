...
func UpdateDNS(cfg *config.Config, addr string, ipVer int) error {
	recordType := "A"
	if ipVer == 6 {
		recordType = "AAAA"
	}

	recordID, err := GetDNSRecordID(cfg, cfg.Cloudflare.ZoneID, cfg.Cloudflare.GameDomainName, recordType)
	if err != nil {
		return err
	}
	...
	url.Path = path.Join(url.Path, "zones", cfg.Cloudflare.ZoneID, "dns_records", recordID)
