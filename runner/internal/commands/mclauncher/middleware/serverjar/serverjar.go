...
type ServerJarMiddleware struct {
	...
	launcherMetaClient *launchermeta.LauncherMeta
}

var _ core.Middleware = (*ServerJarMiddleware)(nil)

func NewServerJarMiddleware(pathProvider env.PathProvider, launcherMetaClient *launchermeta.LauncherMeta) *ServerJarMiddleware {
...
func (m *ServerJarMiddleware) downloadMatchingVersion(c *core.LauncherContext, desiredVersion string, destination string) error {
	versions, err := retry.Retry(func() ([]launchermeta.VersionInfo, error) {
	...
	serverInfo, err := retry.Retry(func() (*launchermeta.ServerInfo, error) {
		return m.launcherMetaClient.GetServerInfo(c.Context(), *matchedVersion)
	}, time.Minute)
	if err != nil {
		return err
	}
	...
	_, err = retry.Retry(func() (_ retry.Void, err error) {
		...
		req, err = http.NewRequestWithContext(c.Context(), http.MethodGet, serverInfo.DownloadURL, nil)
		if err != nil {
			return retry.V, err
		}
