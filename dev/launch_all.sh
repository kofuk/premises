...
exec tmux new-session -s "${name}" -n 'Middleware' 'docker compose up; bash' \; \
     new-window -n 'Redis Console' 'while true; do sleep 3; docker compose exec redis redis-cli; done' \; \
     new-window -n 'PostgreSQL Console' 'while true; do sleep 3; docker compose exec postgres psql -Upremises; done' \; \
