...
import (
	...
	"github.com/kofuk/premises/backend/runner/commands/mclauncher/middleware/world/service"
...
func (m *WorldMiddleware) getRealResourceID(ctx context.Context, worldName string, resourceID string) (string, error) {
	...
	slog.InfoContext(ctx, "Latest world is specified. Looking for the latest revision of the world")
...
func (m *WorldMiddleware) cleanupOldWorldFiles(c core.LauncherContext) error {
	slog.InfoContext(c.Context(), "Cleaning up old world files")
	...
	return func(c core.LauncherContext) error {
		...
		if isNewWorld || worldResourceID != oldResourceID {
			...
			if !isNewWorld {
				...
				slog.InfoContext(c.Context(), "Downloading world from remote")
		...
		if innerError == nil || errors.Is(innerError, core.ErrRestart) {
			slog.InfoContext(c.Context(), "Uploading world to remote")
