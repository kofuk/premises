...
var _ = Describe("Scheduler", func() {
	It("should run tasks in the correct order", func() {
		...
		task1 := scheduler.NewTask(func(ctx context.Context) {
			num.Add(1)
			Expect(int(num.Load())).To(Equal(1))
		}, "Task 1")
		task21 := scheduler.NewTask(func(ctx context.Context) {
			num.Add(1)
			curNum := int(num.Load())
			Expect(curNum).To(Or(Equal(2), Equal(3)))
		}, "Task 2-1", task1.ID())
		task22 := scheduler.NewTask(func(ctx context.Context) {
			num.Add(1)
			curNum := int(num.Load())
			Expect(curNum).To(Or(Equal(2), Equal(3)))
		}, "Task 2-2", task1.ID())
		task3 := scheduler.NewTask(func(ctx context.Context) {
			num.Add(1)
